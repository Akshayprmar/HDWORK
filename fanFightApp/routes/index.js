...
router.get('/wallet/:userId/:entryFee/:discount', async (req, res, next) => {
  ...
  if ( userWallet && user.isValidWallet(userWallet.wallet) ) {
    ...
    // deduct all remaining amount if available deposit is more 
    // else deduct all the available deposit and set it to zero
    if ( userWallet.wallet.deposit >= remainingAmountToBeDeducted ) {
      depositToBeDeducted = remainingAmountToBeDeducted;
      remainingAmountToBeDeducted = 0;
    } else {
      remainingAmountToBeDeducted -= userWallet.wallet.deposit;
      depositToBeDeducted = userWallet.wallet.deposit;
    }
    ...
    if (errorFlag) {
      ...
      userWallet.wallet.deposit -= depositToBeDeducted;
